...
get_ww_data <- function(location_name,
  ...
  ww_clean <- RKI_ww_sites |>
    rename(location = "standort",
                  ...
                  pathogen = "typ",
                  below_LOD = "unter_bg") |>
    select(location, date, state, conc, pop_cov, change_in_lab_indicator,
           normalized, pathogen,
           below_LOD) |>
    filter(state == location_abbr,
    ...
    select(date, site, lab, log_genome_copies_per_ml, log_lod, site_pop,
           below_LOD) |>
