...
The limit of quantification (LOQ) of a measured wastewater concentration (genome copies per unit volume) is calculated as the geometric mean of the limit of quantification for each measured gene for measured sample on a particular date and processed in a particular lab. All values that are flagged in the dataset as having more than half of the measured genes below the limit of detecton are treated as censored in the `wwinference` model, as is described in \[cite model definition\], and the LOQ calculated is used to censor these observations.

In the case where half or fewer than half of the measured genes are below the the limit of detection for a measured sample, the concentration of that sample is calculated as the geometric mean across the measured genes, using half the LOQ as a replacement for the measured gene below the LOQ. This process is described in detail in [@Abunjela\_et_al, @BMChealth, @RKIGithub] (check this).
