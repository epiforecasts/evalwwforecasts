...
Forecasts of COVID-19 hospital admissions will be generated weekly and evaluated against the final observed hospital admissions data, summed across all age groups in Germany during the 2024-2025 respiratory virus season.
...
1. How do characteristics of the wastewater surveillance system impact the relative forecast performance of the wastewater-informed model?
...
This analysis will use publicly available data from RKI of [COVID-19 hospital admissions at the state-level](https://github.com/robert-koch-institut/COVID-19-Hospitalisierungen_in_Deutschland/) and [wastewater concentration data](https://github.com/robert-koch-institut/Abwassersurveillance_AMELAG) at the site-level in Germany during the 2024-2025 respiratory virus season.
...
We will fit and generate forecasts for the number of admissions summed across all age groups, at the state-level.
...
1. CRPS by model summarised across locations and forecast dates, broken down by under/over prediction and dispersion, in the form of a horizontal bar chart for each of the models, for 3 different horizons (7, 14, 28 day horizon).
...
1. scatterplot of CRPS from wastewater model vs CRPS from hospital admissions only model summarised by location

1. CRPS by model stratified by forecast date (stacked bar chart x-axis = date, y-axis = CRPS breakdown, color = model)

1. scatterplot of CRPS from wastewater model vs CRPS from hospital admissions only model summarised by location

1. 50th and 90th interval coverage summarised across locations and forecast dates (horizontal bar chart, color = model)
...
1. population coverage of all the wastewater treatment plants

1. average sampling frequency across sites

1. average latency from last collection date to the forecast date
...
The idea behind this analysis will be that including the CRPS score for the `hosp only` model as an offset accounts for the baseline "difficulty" in that particular location $l$, at forecast date $d$ and horizon $h$.
