...
The limit of quantification (LOQ) of a measured wastewater concentration (genome copies per unit volume) is calculated as the geometric mean of the limit of quantification for each measured gene for measured sample on a particular date and processed in a particular lab.

In the case where half or fewer than half of the measured genes are below the the LOQ for a measured sample, the concentration of that sample is calculated as the geometric mean across the measured genes, using half the LOQ as a replacement for the measured gene below the LOQ. This process is described in detail in [@Abunjela\_et_al, @BMChealth, @RKIGithub] (check this).

## Model specification

We will use the following models to produce forecasts of the 7-day rolling sum of COVID-19 hospital admissions for 28 days ahead of the forecast date:
...
3.  Baseline ARIMA (`arima_baseline`): This model will be fit only to the 7-day rolling sum of hospital admissions at the state level, using the `auto.arima()` function in the `forecast` R package, which will perform an optimization of both the model parameters and the autoregressive order $p$, differencing order $d$ and the moving average order $q$. We intend to use this as a reference point for comparing the magnitude of the absolute scores so that we can put the forecast comparison with and without wastewater in context. It will be calibrated using the entire historical admissions time series.
...
We will use a single evaluation dataset from November X, 2025. It is assume that as of this date, all forecast horizons from the entire period of study (July 1st 2-24 to July 7th 2025) will have reached a stable value. We will evaluate all forecasts using the 7-day rolling sum of hospital admissions. Posterior samples of forecasts will be log-transformed, along with the observations the models are evaluated against. Forecasts will be scored using proper scoring rules, with metrics including the Continuous Ranked Probabilist Score (CRPS), its component breakdown (underprediction, overprediction, dispersion) and interval coverage.

Visual comparisons of forecasts overlaid with evaluation data will be used to assess the forecast performance of each model for individual locations.

We will present the results by analysing the mean scores by model and then stratifying by location, forecast date, and horizon for the models fit to the vintaged hospitalization datasets.

When comparing forecast performance with and without wastewater, we will compute the relative WIS compared to the hospital admissions only `wwinference` model.

Results which include the baseline ARIMA model will be presented in the supplement.

### Model-based evaluation: investigation into drivers of differences in forecast performance with and without wastewater

In addition to stratifying results by forecast date, location, and horizon, we perform an exploratory analysis into the factors that may contribute to the relative performance of the `ww + hosp` model compared to the `hosp only` model.
...
1.  score of the `hosp only` model (as an offset)
2.  number of wastewater treatment plants (sites)
3.  population coverage of all the wastewater treatment plants (proportion of the state population on wastewater)
4.  average sampling frequency across sites (this is likely very similar so may not be useful)
5.  average latency from last collection date to the forecast date
6.  minimum latency from last collection date to forecast date
7.  variability in wastewater data as measured by the coefficient of variation across sites and time during the calibration period

Resulting in the following model formulation:

`CRPS^{ww+hosp}_{h,d,l} ~ \beta + CRPS^{hosponly}_{h,d,l}+ s(numberofsites, k ) + s(popcoverageww, k ) + s(avgfreqsampling, k) + s(avglatencyreporting,k) + s(minlatencyreporting, k) + s(variability_wwdata)`
...
Figure 2. Schematic diagram describing the retrospective production of forecasts and the evaluation procedure

## Visual comparison of forecast performance

Q: Do we include baseline in all of the below? Probably not necessary in main text but could make supplement version where we include it

Figure 3. Visual comparison of fit and forecast for a subset of locations and forecast dates (can pick ones where ww improves, wastewater has no impact, wastewater degrades performance) + fit to wastewater data

3 rows, 4 columns

row = location

column 1 = hospital admissions fit with `ww+hosp` vs `hosp_only`

columns 2:4 = fit to wastewater data by site

Figure 4. Visual comparison of forecast performance across forecast dates for a subset of locations for horizons 1-7

2 rows, 3 columns

row 1 = forecast eval overlay `ww+hosp` vs `hosp only`

row 2 = score underlay (bar chart)

columns = locations

Supplement:

1.  Fig 3 & 4 with baseline ARIMA
2.  Fig 4 for additional horizons (7-14, 14-21, 21-28)

## Quantitative forecast comparison

Figure 5. "traditional" score comparison: overall, by horizon, by location, by forecast date, by location and forecast date

A. CRPS by model summarised across locations and forecast dates, broken down by under/over prediction and dispersion, in the form of a horizontal bar chart for each of the models across all 28 day horizons.

C. 50th and 90th interval coverage summarised across locations and forecast dates (horizontal bar chart, color = model)

B. rWIS by horizon overall

D. CRPS by model stratified by location ordered by CRPS of the hosp-only model (stacked bar chart x-axis = location, y-axis = CRPS breakdown, color = model, shading = CRPS breakdown)

E. CRPS by model stratified by forecast date (stacked bar chart x-axis = date, y-axis = CRPS breakdown, color = model, shading = CRPS breakdown)

F. National hospital admissions over time

G. Heatmap of rWIS (relative to hosp only) averaged across horizons for each forecast date and location (x-axis = forecast date, y-axis = location, color = rWIS)

## Investigation into drivers of differences in forecast performance with and without wastewater

Result table from GAM output (what is $\beta$)

Figure 6: Model based eval results

A. scatterplot of CRPS from wastewater model vs CRPS from hospital admissions only model summarised by location-forecast date

B. scatterplot of CRPS from wastewater vs hosp only summarised by location

C. scatter plot of CRPS from wastewater vs hosp only summarise by forecast date

D. Partial effect of min latency, avg latency, avg frequency, pop coverage, variability in ww, number of sites

## Extensions to still consider:
